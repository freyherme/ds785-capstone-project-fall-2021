...
models_completed = pd.read_csv(csv_file, usecols = ["model_name", "kernel"])
...
def check_existing_models(current_model_name, current_solver):
    for i, m in models_completed.iterrows():
        if m['model_name'] == current_model_name and m['kernel'] == current_solver:
            return True
...
for model_name, model_dict in models_dict.items():
    ...
    for kernel in model_dict['kernel']:
        ...
        if check_existing_models(model_name, kernel):
            print(f". . . . . . . . . . Skipping  {model_name}, kernel: {kernel}. . . . . . . . . . . .")
            continue
