#############################################################################################
########         Support Vector Machine w/ Calibration - Plot ROC Curve              ########
#############################################################################################
...
csv_file = 'svm_model_evals_clean.csv'
...
###          Load Model           ###
#####################################
svm_model = pickle.load(open('client_health_modeling/svm_full_cv_callibrated_model_obj_clean.sav', 'rb'))
...
for cf in covid_filter:
    ...
    model_filename = f"svm_{model_name}_model_obj_clean_callibrated_split.sav"
    pickle.dump(calibrated_svm_model, open(model_filename, 'wb'))

    # cross_validate(estimator = model, X = X, y = y, cv = k, n_jobs = 15,
    ...
    if 'feature_selection' in models_dict[model_name]:
        if models_dict[model_name]['feature_selection'][0] == 'SelectKBest':
            ...
            encoderObj = calibrated_svm_model.steps[0][1].transformers_[1][1].named_steps['onehot']
            ...
            kbest_did_select = calibrated_svm_model.steps[1][1].get_support()
    ...
    preds = calibrated_svm_model.predict(x_test)
    predicted_probs = calibrated_svm_model.predict_proba(x_test)
    ...
    with open(csv_file, 'a') as f:
        ...
        writer.writerow([datetime.today().strftime('%Y-%m-%d-%H:%M:%S'),
                         ...
                         best_threshold_accuracy, round(auc_score, 4), best_threshold_f1, best_threshold_precision,
