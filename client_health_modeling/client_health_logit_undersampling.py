################################################################################
########            Logistic Regression w/ Under Sampling               ########
################################################################################
...
csv_file = 'logit_model_eval_undersampling_wo_weights.csv'
models_completed = pd.read_csv(csv_file, usecols = ["model_name", "solver"])
...
def check_existing_models(current_model_name, current_solver):
    for i, m in models_completed.iterrows():
        if m['model_name'] == current_model_name and m['solver'] == current_solver:
            return True
...
for model_name, model_dict in models_dict.items():
    ...
    for solver in model_dict['solver']:
        ...
        if check_existing_models(model_name, solver):
            print(f". . . . . . . . . . Skipping  {model_name}, solver: {solver}. . . . . . . . . . . .")
            continue

        print(f"""
        ##############################################################################
                                {model_name}, solver: {solver}                   
        ##############################################################################
        """)
        ...
        if include_class_weights:
            lor = LogisticRegression(C = 1.0, random_state = 1, solver = solver,
        ...
        #########################################################
        ## Perform UnderSampling using Nearest Neighbor Method ##
        #########################################################
        ...
        # Add Feature Selection to Data Pipeline if Necessary
        ...
        for cf in covid_filter:
            ...
            # Set Stratified Cross_validation parameters
            ...
            cv_probs = cross_val_predict(estimator = model, X = X, y = y, cv = k, n_jobs = 15, method = 'predict_proba')

            # define thresholds
            thresholds = arange(0, 1, 0.01)
            # evaluate each threshold
            scores = [metrics.f1_score(y, to_labels(cv_probs[:, 1], t)) for t in thresholds]
            # get best threshold
            ix = argmax(scores)
            print('Threshold=%.3f, F-Score=%.5f' % (thresholds[ix], scores[ix]))

            best_threshold = thresholds[ix]
            cv_preds = [1 if prob > best_threshold else 0 for prob in cv_probs[:, 1]]

            cv_accuracy = round(metrics.accuracy_score(y, cv_preds), 4)
            cv_auc = round(metrics.roc_auc_score(y, cv_preds), 4)
            cv_f1 = round(metrics.f1_score(y, cv_preds), 4)
            cv_precision = round(metrics.precision_score(y, cv_preds), 4)
            cv_recall = round(metrics.recall_score(y, cv_preds), 4)
            ...
            with open(csv_file, 'a') as f:
                ...
                writer.writerow([datetime.today().strftime('%Y-%m-%d-%H:%M:%S'),
                                 ...
                                 best_threshold,
                                 cv_accuracy, cv_auc, cv_f1, cv_precision, cv_recall, solver, cf]
                                )
